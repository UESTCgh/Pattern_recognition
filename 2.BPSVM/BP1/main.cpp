...
// 从 CSV 文件中读取训练数据集
bool loadTrainingData(const std::string& filePath, std::vector<std::vector<double>>& X, std::vector<std::vector<double>>& y) {
...
void crossValidation(const std::vector<std::vector<double>>& X, const std::vector<std::vector<double>>& y,BPNeuralNetwork& bpnn,const std::string& metricsFilePath) {
    ...
    // 输出平均性能指标
    std::cout << "Average SE: " << total_se << std::endl;
    std::cout << "Average SP: " << total_sp << std::endl;
    std::cout << "Average ACC: " << total_acc  << std::endl;
    std::cout << "Average AUC: " << total_auc  << std::endl;
...
int main() {
    std::string inputFilePath = "E:/GitHub/AI_VI/2.BPSVM/BP1/data/data.csv";
    std::string outputFilePath = "E:/GitHub/AI_VI/2.BPSVM/BP1/data/filtered_data.csv";
    std::string invalidFilePath = "E:/GitHub/AI_VI/2.BPSVM/BP1/data/invalid_data.csv";

    std::string trainFilePath = "E:/GitHub/AI_VI/2.BPSVM/BP1/data/train.csv";
    std::string testFilePath = "E:/GitHub/AI_VI/2.BPSVM/BP1/data/test.csv";

    std::string lossFilePath = "E:/GitHub/AI_VI/2.BPSVM/BP1/data/loss_values.csv";

    std::string misclassifiedFilePath = "E:/GitHub/AI_VI/2.BPSVM/BP1/data/result/misclassified_samples.csv";

    std::string modelFilePath = "E:/GitHub/AI_VI/2.BPSVM/BP1/data/result/models.txt";
    std::string matFilePath = "E:/GitHub/AI_VI/2.BPSVM/BP1/data/mat.csv";

    // 使用 BP 神经网络模型的特征：身高、体重、鞋码、50m 成绩
    ...
    std::cout << "数据集已按照 7:3 比例划分为训练集和测试集，并已保存到文件中。" << std::endl;

    //3.初始化 BP 神经网络
    std::cout << "BP神经网络初始化..." << std::endl;
    BPNeuralNetwork bpnn(input_size, hidden_size, output_size, learning_rate);
    // 构建训练数据集，读取 train.csv 文件
    std::vector<std::vector<double>> X;
    std::vector<std::vector<double>> Y;
    if (!loadTrainingData(trainFilePath, X, Y)) {
        return 1;
    }
    //printData(X,Y);
    ...
    if (!loadTrainingData(testFilePath, X_test, Y_test)) {
        return 1;
    }
    ...
    std::cout << "开始训练 BP 神经网络..." << std::endl;
